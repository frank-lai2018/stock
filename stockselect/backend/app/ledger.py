...
單位：lots=張（1張=1000股）、price=每股價、fee/tax=該筆總額（元）。
...
def build(txns):
    """txns：同一檔的交易 list[dict]（含 id/action/trade_date/lots/price/fee/tax）。
    回傳：未平倉彙總 + 已實現明細（closed round-trips）。"""
    ordered = sorted(txns, key=lambda t: (t["trade_date"], 0 if t["action"] == "buy" else 1, t["id"]))
    fifo = []          # 未配對買批：[lots, price, date, fee_per_lot]
    realized = 0.0
    closed = []
    buy_lots = sell_lots = 0.0
    for t in ordered:
        lots = _f(t["lots"]); price = _f(t["price"])
        fee = _f(t.get("fee")); tax = _f(t.get("tax"))
        if lots <= 0:
            continue
        if t["action"] == "buy":
            buy_lots += lots
            fifo.append([lots, price, t["trade_date"], fee / lots])
        else:  # sell（FIFO 配對最早的買批）
            sell_lots += lots
            remain = lots
            sell_cost_per = (fee + tax) / lots      # 賣出成本攤每張
            while remain > 1e-9 and fifo:
                lot = fifo[0]
                take = min(remain, lot[0])
                buy_price, buy_fee_per = lot[1], lot[3]
                pnl = (price - buy_price) * take * 1000 - (sell_cost_per + buy_fee_per) * take
                realized += pnl
                days = (t["trade_date"] - lot[2]).days
                closed.append({
                    "lots": round(take, 3), "buy_date": lot[2].isoformat(), "buy_price": buy_price,
    ...
    net_lots = sum(l[0] for l in fifo)
    cost_remaining = sum(l[0] * l[1] for l in fifo)          # 每股 × 張（未 ×1000）
    avg_cost = (cost_remaining / net_lots) if net_lots > 1e-9 else None
    return {
        "net_lots": round(net_lots, 3),
        "avg_cost": round(avg_cost, 4) if avg_cost is not None else None,
        "realized_pnl": round(realized),
        "closed": closed,
        "buy_lots": round(buy_lots, 3), "sell_lots": round(sell_lots, 3),
    }
