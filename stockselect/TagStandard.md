...
| **RS 評等 `rs_rating`** | `rs_raw` 的百分位 × 100（0–99，越大越強勢；Minervini 門檻 70）。**依證券類別分池排名**（股票 vs 股票、ETF vs ETF），避免 ETF 混入扭曲個股 RS |
...
每列一筆買/賣（含日期、股數、價、手續費、證交稅）。未平倉持股與已實現損益由 [`backend/app/ledger.py`](backend/app/ledger.py)
的 **FIFO 引擎**推導：賣單依買進先進先出配對，買進手續費攤入成本、賣出手續費/稅於平倉時實現。
診斷對象＝各檔的未平倉部位（股數＝買−賣、成本＝剩餘買批加權均價）。
...
> 未實現損益：`未平倉股數×(現價−均成本)`；已實現：FIFO 平倉逐筆 `(賣價−買價)×股數 − 手續費 − 證交稅`；
