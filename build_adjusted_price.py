...
輸入：
   ...
   股利：  <div-root>\<股號>\<股號>_dividend.csv （fetch_fundamentals 的 dividend 資料集）
輸出：
   <price-root>\<股號>\<股號>_adj.csv
   欄位：date, open/high/low/close(原始), volume, adj_open/high/low/close(還原), cumfactor(還原因子)

還原方法（後復權，最新一天=原始價，往前調整使序列連續）：
   除息參考價 = (前收 - 現金股利) / (1 + 股票股利/10)
   因子 r = 參考價 / 前收；除權息日「之前」的所有價格乘上其後所有 r 的累積。
...
def parse_rows(text, skip_header):
    ...
    for cells in reader:
        ...
        if d is None or len(cells) < 7:                # 標題/欄名/說明列 → 跳過
            continue
        rows.append({"date": d, "volume": to_num(cells[1]),
...
def adjust(df, events):
    """後復權：算每日 cumfactor 與還原 OHLC。"""
    df = df.copy()
    df["ratio"] = 1.0
    for ex_date, (cash, stock) in sorted(events.items()):
        ...
        if not prev_close > 0:
            continue
        ref = (prev_close - cash) / (1.0 + stock / 10.0)   # 除權息參考價
        r = ref / prev_close
        tgt = df.index[df["date"] >= ex_date]          # 除權息日那一列（無則取其後第一個交易日）
        if len(tgt) == 0 or not r > 0:
            continue
...
def build_one(code, price_root, div_root):
    ...
    if prices.empty:
        ...
        return
    events = load_dividends(os.path.join(div_root, code, f"{code}_dividend.csv"))
    out = adjust(prices, events)
    ...
    print(f"  [存] {code} {len(out)} 列、{len(events)} 個除權息事件 → {code}_adj.csv")
