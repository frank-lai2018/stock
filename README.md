...
| `daily_update.py` | **每日一鍵**：串起 update_prices → build_adjusted_price → load_to_db --since | 綜合（收盤後跑）|
...
**每日一條龍**（收盤後）：上面三步已包成一支 **`daily_update.py`**，跑一行即可：
```bash
python daily_update.py                                 # 更新「今天」→ 還原 → 入庫
python daily_update.py --date 2026-07-11               # 指定交易日
python daily_update.py --date 2026-07-11 --dry-run     # 步驟 1、2 照做；步驟 3 只驗證不寫 DB
python daily_update.py --skip-load                     # 只更新 CSV（完全不碰 DB）
python daily_update.py --only 2330 5483                # 只跑幾檔（測試/補單檔）
```
它依序做（順序不可顛倒）：
1. `update_prices.run` 抓當天全市場 → 併入各股月檔，**回傳當天有更新的股號**；
2. 只對「有更新的股」重算 `_adj.csv`（含最新日，效率比全掃高）；
3. `load_to_db --since <日>` 只把當天 upsert 進 DB（預設只灌 `price_daily`，`--tables` 可調）。
> ⚠️ 為什麼要照順序：跳過第 2 步，`load_to_db` 讀不到當天的還原價，`price_daily` 就缺這一天。非交易日第 1 步回空 → 自動跳過 2、3。
> 連線：設環境變數 `DATABASE_URL` 或帶 `--dsn`（`--dry-run` / `--skip-load` 不需要）。
