...
### 步驟 0：抓全市場代碼清單
```bash
python fetch_stock_codes.py --out 台股股票代碼NEW.xlsx            # 只含普通股（同舊行為）
python fetch_stock_codes.py --type all --out 台股股票代碼NEW.xlsx  # 普通股 + ETF（見下方 ETF 回補）
```
產出欄位：股票代碼、中文名稱、市場別、上市櫃日期、產業別、**證券類別**（stock/etf）。
`--type` 可選 `stock`(預設) / `etf` / `all`。ETF 用「分類標題列」辨識，穩定不靠猜 CFICode。
...
### 步驟 1.5：ETF 回補（一次性；每日之後自動接續）
ETF 的價量走現有「按日全市場」管線（TWSE MI_INDEX / TPEx EW 本就含 ETF），只需一次性把歷史補齊、
把 ETF 併入 `stock` 主檔。三大法人 / 融資 / 集保大戶也一併涵蓋；基本面（EPS/財報/營收）ETF 無此資料。
```bash
# ① 既有 DB 加欄位（區分 stock / etf）
psql -U frank -d twstock -c "ALTER TABLE stock ADD COLUMN IF NOT EXISTS security_type VARCHAR(8) DEFAULT 'stock';"

# ② 產出主檔(含ETF) + ETF 代碼清單
python fetch_stock_codes.py --type all --out 台股股票代碼NEW.xlsx
python fetch_stock_codes.py --type etf --out 台股ETF代碼.xlsx --codes-out etf_codes.txt

# ③ 回補歷史日K：依上市日、分市場自動分流（上市 floor 2010-01，與股票一致）
python batch_download.py --xlsx 台股ETF代碼.xlsx                 # 上櫃也要 2010-01 → 加 --start-otc 2010-01
#   先測水溫：python batch_download.py --xlsx 台股ETF代碼.xlsx --limit 3

# ④ 建還原價（ETF 暫無配息 → adj=原始價；配息還原見未來 Phase 3）
python build_adjusted_price.py --codes-file etf_codes.txt

# ⑤ 入庫（stock 主檔含 ETF + ETF 的 price_daily）
python load_to_db.py --xlsx 台股股票代碼NEW.xlsx --codes-file etf_codes.txt --tables stock,price_daily

# ⑥ 重建選股視圖（ETF 納入快照）
psql -U frank -d twstock -f stockselect\sql\mv_stock_snapshot.sql
```
- 之後**每日更新自動接續**：ETF 資料夾已存在，`nightly.py` / `update_prices.py` 預設模式就會更新；
  主檔用 `--type all` 版 xlsx，`daily_update` 的入庫也會含 ETF。
- 新工具旗標：`fetch_stock_codes --type/--codes-out`；`download_twse_csv / download_tpex_csv / build_adjusted_price / load_to_db --codes-file`。
...
`daily_update.py` 顧每日資料（股價 + 法人/融資/PER/外資 + 大盤/櫃買指數）。其餘低頻基本面依頻率各有工具：
...
```
> 歷史回補：TDCC opendata 只有最新一週；FinMind 的 `TaiwanStockHoldingSharesPer` 是**付費層(backer/sponsor)限定**（免費 token 回 400），故免費無法補歷史。務實做法＝由 `nightly.py` 每晚存 `H:\data\Holders` 週快照，往後自建歷史。
...
python nightly.py --dsn "postgresql://帳號:密碼@localhost:5432/twstock" #每晚跑的
...
| 工作 | 何時跑 | 防重 |
|---|---|---|
| daily（股價+法人+融資+PER+外資+大盤/櫃買指數）| 每晚（呼叫 daily_update；非交易日自動跳過）| — |
| holderdist（集保股權分散/大戶）| 每晚檢查（TDCC 週資料，自動抓最新週 + 存快照）| idempotent，無新週則原地覆蓋 |
...
- **日常就一條**：`python nightly.py --dsn ...`（或設排程）。落後多天造成斷洞時，才對漏掉的交易日各跑 `daily_update.py --date`。
