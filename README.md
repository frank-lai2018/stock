...
| `update_fundamentals.py` | **低頻**財報/EPS/股利更新：串 `fetch_fundamentals → load_to_db`（季/年跑）| FinMind，**需 token** |

> 需求：`pip install pandas lxml openpyxl`（下載器本身純標準庫，代碼清單 / 批次驅動 / 基本面 / 還原才需要這些）；入庫另需 `pip install psycopg2-binary`。

> **兩種資料更新管道**：① 每日的價量與籌碼（股價/法人/融資/PER）走**證交所 by-date**（`daily_update.py` + `update_chips.py`，快、免 token）；② 低頻的財報/EPS/股利走 **FinMind 逐檔**（`update_fundamentals.py`，季/年才跑一次，成本可接受）；月營收走**官方 OpenAPI by-date**（`update_revenue.py`）。
...
**③ 財報 / EPS / 股利（每季 / 每年）— `update_fundamentals.py`**
低頻資料沿用 FinMind 逐檔（季/年才跑一次，成本可接受）；串「fetch → load」一鍵：
```bash
...
```
- `--start` 給近一年即可（只重抓/入庫近期）；需 `FINMIND_TOKEN`。preset：`quarterly`（財報+EPS）/ `dividend`（股利）/ `revenue` / `all`。
