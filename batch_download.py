...
每檔起抓月份會「看上市櫃日期」自動決定：
   起抓月 = max(上市櫃日期的月份, --start)
   → 上市櫃日期晚於 --start 就從上市櫃當月開始；早於就從 --start（預設 2010-01）開始。
   這樣不會對每檔都從 2010 硬抓一堆上市前的「無資料」月份。
...
用法：
  ...
  python batch_download.py --out "H:\data" --start 2010-01 --delay 4

提醒：
- 需要 Excel 有「市場別」欄（值為 上市/上櫃）才能自動分流；沒有的話全部當『上市』走 TWSE。
- 需要 Excel 有「上市櫃日期」欄才能依上市日起抓；沒有的話全部從 --start 開始（等同舊行為）。
- --start 是「最早回溯月份」(floor)：只有上市櫃日期比它早的檔，才會被拉回到 --start。
...
def main():
    ...
    ap.add_argument("--start", default="2010-01", help="最早回溯月份 YYYY-MM（floor，預設 2010-01）")
    ...
    print(f"起抓月份：每檔取 max(上市櫃日期, {args.start})" + ("" if has_date else "（無日期欄→全部用 floor）"))
    ...
    for i, (code, market, list_date) in enumerate(rows, 1):
        ...
        if not script:                                # 興櫃等不支援 → 跳過
            ...
            skipped += 1
            continue
        start = effective_start(list_date, floor_ym)
